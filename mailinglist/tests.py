...
class MockSendMailToSubscriberTask:
    def setUp(self):
...
class SubscriberCreationTestCase(MockSendMailToSubscriberTask,
                                        TestCase):
...
class SubscriberManagerTestCase(TestCase):
    def testConfirmedSubscribersForMailingList(self):
        ...
        confirmed_users_qs = Subscriber.objects.\
            confirmed_subscribers_for_mailing_list(mailing_list=mailing_list)
        self.assertEqual(len(confirmed_users), len(confirmed_users_qs))
